    ...
    internal static class Extensions
    {
        internal static string GetValue(this FormatType formatType) =>
            formatType switch
            {
                ...
                FormatType.COMPACT_DECODED => "COMPACT-DECODED",
                _ => throw new Exception($"Unknown FormatType encountered: {formatType}"),
            };

        internal static string GetValue(this SearchQueryType searchQueryType) =>
            searchQueryType switch
            {
                SearchQueryType.DMQL => "DMQL",
                SearchQueryType.DMQL2 => "DMQL2",
                _ => throw new Exception($"Unknown SearchQueryType encountered: {searchQueryType}")
            };
